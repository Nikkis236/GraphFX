...
public class GraphPane {
    ...
    public GraphPane() {
        ...
        pane.setStyle("-fx-background-color: #c0cedb");
        configurePane();
    }
...
//////////////////////////////////////////КРАТНЫЕ ДУГИ//////////////////////////////////////////////////
                if ((graphController.getArcs().contains(arc))
                    ...
                    DrawableArc arcShape = new DrawableArc(arc, beginForArc, endForArc,1);

                    drawableArcs.add(arcShape);
                    pane.getChildren().addAll(arcShape.getArrow(), arcShape.getCurve());

                    pane.getChildren().remove(arcShape.getArrow());
                    Arc reversedArc = new Arc(arcShape.getSourceArc().getEnd(), arcShape.getSourceArc().getBegin());
                    reversedArc.setDirected(false);
                    graphController.addArc(reversedArc);
                    arcShape.getSourceArc().setDirected(false);
    ...
    private EventHandler<KeyEvent> nodeOrArcRemovingEventHandler = e -> {
        for (DrawableNode drawableNode : drawableNodes) {
            if (drawableNode.isFocused()) {
                ...
                for (DrawableArc drawableArc : arcsToRemove) {
                    pane.getChildren().removeAll(drawableArc.getLine(), drawableArc.getArrow(), drawableArc.getCurve());
                }
                return;
            }
        }
        ...
        for (DrawableArc drawableArc : drawableArcs) {
            if (drawableArc.isFocused()) {
                ...
                pane.getChildren().removeAll(drawableArc.getLine(), drawableArc.getArrow(), drawableArc.getLoop(), drawableArc.getCurve());
                return;
            }
        }
    };
        ...
        for (DrawableArc drawableArc : drawableArcs) {
            if (drawableArc.isFocused()) {
                colorPicker.setOnAction(actionEvent -> {
                    ...
                    //drawableArc.getCurve().setStroke(colorPicker.getValue());
